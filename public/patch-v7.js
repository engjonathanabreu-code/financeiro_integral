/* Integral Financeiro V7.1 - autenticação compartilhada com ERP Integral */
(function(){
  ...
  window.IntegralERP={sb,profiles:[],projects:[],plans:[],steps:[],responsibles:[],loaded:false,error:''};
  ...
  async function loadERP(){
    ...
    try{
      ...
      if(profiles.length){
        db.usersMvp=profiles.filter(x=>x.ativo!==false).map(x=>({id:hashId(x.id),erpId:x.id,name:x.nome,email:x.email||'',role:roleOf(x.tipo),sector:x.tipo||'Administrativo',active:true,source:'ERP'}));
      }
      db.erpProjects=projects.map(x=>({id:x.id,name:x.nome,status:x.status}));
      db.erpPlans=plans.map(p=>{
        const stepIds=steps.filter(s=>s.plano_id===p.id).map(s=>s.id);
        const userIds=responsibles.filter(r=>stepIds.includes(r.etapa_id)).map(r=>r.usuario_id);
        const sectors=[...new Set(profiles.filter(u=>userIds.includes(u.id)).map(u=>u.tipo).filter(Boolean))];
        return {id:p.id,title:p.titulo,projectId:p.projeto_id||'',status:p.status,sectors};
      });
      save();
    }catch(e){
      window.IntegralERP.loaded=true;window.IntegralERP.error=e.message||String(e);
      console.warn('Integração ERP parcial:',e);
    }
  }
  ...
  function setFinanceUser(p,authUser){
    ...
    if(!exists){
      db.usersMvp=db.usersMvp||[];
      db.usersMvp.push({id:hashId(p.id),erpId:p.id,name:p.nome,email:user.email||'',role:user.role,sector:p.tipo||'',active:true,source:'ERP'});
      save();
    }
  }
  ...
  function renderERPLogin(error=''){
    ...
    $('#erpFinLogin').onsubmit=async e=>{
      e.preventDefault();
      const btn=e.submitter||e.target.querySelector('button[type="submit"]'),err=$('#erpErr');
      btn.disabled=true;btn.textContent='Entrando...';err.textContent='';
      try{
        const email=$('#erpEmail').value.trim().toLowerCase(),password=$('#erpPass').value;
        const {data,error}=await sb.auth.signInWithPassword({email,password});
        if(error)throw error;
        const p=await profileFor(data.user);
        setFinanceUser(p,data.user);
        view=user.role==='Administrador'?'dashboard':'budgets';matrix(false);app();
        loadERP().then(()=>{if(document.querySelector('.shell'))app();});
      }catch(x){
        console.error('Login Financeiro/ERP:',x);
        try{await sb?.auth.signOut()}catch{}
        err.textContent=x.message==='Invalid login credentials'?'E-mail ou senha incorretos. Use os mesmos dados do ERP.':(x.message||'Falha ao entrar.');
  ...
  login=async function(){
    try{
      ...
      if(!sb)return renderERPLogin('Não foi possível carregar a conexão com o ERP.');
      const {data:{session},error}=await sb.auth.getSession();
      if(error)throw error;
      if(!session)return renderERPLogin();
      const p=await profileFor(session.user);
      setFinanceUser(p,session.user);
      view=user.role==='Administrador'?'dashboard':'budgets';matrix(false);app();
      loadERP().then(()=>{if(document.querySelector('.shell'))app();});
    }catch(e){
      console.error('Sessão Financeiro/ERP:',e);
      try{await sb?.auth.signOut()}catch{}
      renderERPLogin(e.message||'Não foi possível validar sua sessão do ERP.');
    }
  };

  function planChoices(sector=''){
    const plans=db.erpPlans||[];
    return plans.filter(p=>!sector||!p.sectors?.length||p.sectors.some(s=>norm(s)===norm(sector)));
  }

  function enhanceBudgetModal(modal){
    const labels=[...modal.querySelectorAll('label')],planLabel=labels.find(l=>l.textContent.includes('Plano de Trabalho ERP')),sector=modal.querySelector('select[name="sector"]');
    if(planLabel){
      const old=planLabel.parentElement.querySelector('input[name="erp"]');
      if(old){
        const sel=document.createElement('select');sel.name='erp';
  ...
  const observer=new MutationObserver(()=>document.querySelectorAll('.modal-backdrop .modal').forEach(m=>{if(!m.dataset.erpEnhanced&&m.textContent.includes('Plano de Trabalho ERP')){m.dataset.erpEnhanced='1';enhanceBudgetModal(m)}}));
  observer.observe(document.documentElement,{childList:true,subtree:true});

  function decorateERP(){
    const c=$('#content');if(!c)return;
    c.querySelector('.erp-sync-chip')?.remove();
    const chip=document.createElement('div');chip.className='erp-sync-chip';
    chip.innerHTML=window.IntegralERP.loaded?`<span class="badge ${window.IntegralERP.error?'warn':'ok'}">ERP ${window.IntegralERP.error?'parcial':'conectado'}</span><span>${db.usersMvp?.length||0} usuários • ${db.erpPlans?.length||0} planos • ${db.erpProjects?.length||0} projetos</span>`:`<span class="badge">ERP conectado</span><span>Sincronizando cadastros...</span>`;
    c.prepend(chip);
  }
  ...
  users=function(){
    ...
    $('#content').innerHTML=`<div class="erp-sync-chip"><span class="badge ok">Login integrado</span><span>Usuários e senhas são os mesmos do ERP Integral.</span></div><div class="table-wrap excel-wrap"><table class="table excel-table"><thead><tr><th>Nome</th><th>E-mail</th><th>Perfil/Setor</th><th>Status</th></tr></thead><tbody>${(db.usersMvp||[]).map(u=>`<tr><td><b>${esc(u.name)}</b></td><td>${esc(u.email||'')}</td><td>${esc(u.sector||'')}</td><td><span class="badge ok">Ativo</span></td></tr>`).join('')}</tbody></table></div>`;
  };
