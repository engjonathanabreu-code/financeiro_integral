...
function guard(){const buttons=[...document.querySelectorAll('.nav [data-view="receivables"]')];buttons.forEach(b=>{if(allowed()){b.hidden=false;b.style.removeProperty('display')}else if(!profilePending()){b.hidden=true;b.style.setProperty('display','none','important')}});const v=(()=>{try{return typeof view!=='undefined'?view:window.view}catch{return window.view}})();if(!allowed()&&!profilePending()&&v==='receivables'){try{view='dashboard'}catch{}window.view='dashboard';const fn=window.IntegralFinanceRouter?.render;if(typeof fn==='function')fn('dashboard')}}
