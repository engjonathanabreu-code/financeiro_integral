...
function allowed(){
 ...
 if(role==='administrador'||role==='financeiro'||sector==='financeiro')return true;
 /* O perfil do ERP é hidratado de forma assíncrona. Depois que o módulo de permissões
    identifica o colaborador como Financeiro, este guard deve respeitar essa decisão. */
 try{if(financeAccessApi()?.isFinance?.())return true}catch{}
 return false;
}
function profilePending(){
 const u=currentUser();
 if(!u)return false;
 /* Funcionários vindos do ERP podem chegar primeiro apenas com role/nome e receber
    o setor alguns milissegundos depois. Nesse intervalo não redirecionamos a rota. */
 const role=norm(u?.role||u?.tipo||u?.type);
 const sector=norm(u?.setor||u?.sector);
 return role!=='administrador'&&!sector&&!u?.erpId&&!!window.IntegralERP?.sb;
}
function guard(){
 const buttons=[...document.querySelectorAll('.nav [data-view="recebimentos"],.nav [data-view="receipts"]')];
 buttons.forEach(b=>{if(allowed()){b.hidden=false;b.style.removeProperty('display')}else if(!profilePending()){b.hidden=true;b.style.setProperty('display','none','important')}});
 const v=(()=>{try{return typeof view!=='undefined'?view:window.view}catch{return window.view}})();
 if(!allowed()&&!profilePending()&&(v==='recebimentos'||v==='receipts')){
...
document.addEventListener('click',e=>{
 const b=e.target.closest?.('[data-view="recebimentos"],[data-view="receipts"]');
 if(!b)return;
 if(allowed()||profilePending())return;
 e.preventDefault();e.stopImmediatePropagation();guard();
},true);
